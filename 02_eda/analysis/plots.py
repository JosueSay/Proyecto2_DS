...
def plotBeforeAfterCosine(sim_before: pd.DataFrame, sim_after: pd.DataFrame):
    ...
    ax.hist(sim_before["cosine_tfidf"], bins=40, alpha=0.6, label="antes")
    ax.hist(sim_after["cosine_tfidf"], bins=40, alpha=0.6, label="después")
...
def plotBeforeAfterJaccard(sim_before: pd.DataFrame, sim_after: pd.DataFrame):
    fig, ax = plt.subplots(figsize=(7,4))
    ax.hist(sim_before["jaccard_tokens"], bins=40, alpha=0.6, label="antes")
    ax.hist(sim_after["jaccard_tokens"], bins=40, alpha=0.6, label="después")
...
def plotTruncationBars(trunc_before: dict, trunc_after_overall: dict):
    ...
    w = 0.38
    fig, ax = plt.subplots(figsize=(7,4))
    ax.bar(x - w/2, before_vals, width=w, label="antes")
    ax.bar(x + w/2, after_vals,  width=w, label="después")
...
def plotHistTailCompare(file_a: str, file_b: str, split: str):
    ...
    ax.bar(x-w/2, tails_a, width=w, label="A")
    ax.bar(x+w/2, tails_b, width=w, label="B")
...
def plotLeakageBars(leakage: dict):
    if not leakage: return
    pu_train = leakage.get("prompts_unique_train"); pu_valid = leakage.get("prompts_unique_valid")
    inter = leakage.get("prompts_intersection_count"); pct = leakage.get("prompts_intersection_pct")
    fig, ax = plt.subplots(figsize=(6,3.5))
    ax.bar(["train","valid","inter"], [pu_train or 0, pu_valid or 0, inter or 0])
    ax.set_title(f"fuga de prompts (inter={pct or 0:.2f}%)")
    ax.set_ylabel("conteo")
...
def plotSimilarityScatter(sim_before: pd.DataFrame, sim_after: pd.DataFrame):
    ...
    def scatter(df, title, name):
        ...
        ax.scatter(x, cos, s=6, alpha=0.35)
        ax.set_title(title)
